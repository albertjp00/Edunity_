...
export class AdminRepository implements IAdminRepository {
  ...
  async verifyKyc(id: string): Promise<void | null> {
    return await InstructorModel.findByIdAndUpdate(id, {
      KYCstatus: "verified",
      KYCApproved: true,
    });
  }
    ...
    try {
      ...
      const mapped = await Promise.all(
        purchases.map(async (purchase) => {
          const user = await UserModel.findById(purchase.userId).select(
            "name email",
          );
          const course = await CourseModel.findById(purchase.courseId).select(
            "title price",
          );
  ...
  async getEarningsData(
    ...
    try {
      ...
      const limit = 4;
      const total = await EarningModel.countDocuments(filter);
      const totalPages = Math.ceil(total / limit);
      const skip = (page - 1) * limit;
      const earnings = await EarningModel.find(filter).sort(sort).skip(skip).limit(limit);
  ...
  async blockCourse(courseId: string): Promise<boolean | null> {
    try {
      ...
      if (!course?.blocked) {
        ...
        await MyCourseModel.updateMany(
          { courseId: courseId },
          { $set: { blocked: true } },
        );
      } else {
