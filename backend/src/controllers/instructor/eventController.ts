    ...
    IEventParticipationController
{
  ...
  getAllEvents = async (
    ...
    try {
      ...
      const result = await this._eventService.getMyEventsRequest(
        id as string,
        search,
        page,
      );
  ...
  getEvent = async (
    ...
    try {
      ...
      res.status(HttpStatus.OK).json({ success: true, event: result?.event });
    } catch (error) {
      console.log(error);
      next(error);
    }
  ...
  joinEvent = async (
    ...
    try {
      ...
      const result = await this._eventService.joinEventRequest(
        eventId,
        instructorId,
      );
  ...
  endEvent = async (
    ...
    try {
      ...
      const result = await this._eventService.endEventRequest(
        eventId,
        instructorId,
      );
      ...
      if (!result || !result.success) {
        res
          .status(400)
          .json({ message: result?.message || StatusMessage.FAILED_TO_END });
        return;
      }
