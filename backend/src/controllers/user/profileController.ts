...
import { ChangePasswordInputDto, EditProfileInputDto } from "../../dto/inputUserDto";
    ...
    INotificationController
{
    ...
    try {
      ...
      if (profile) {
        ...
        res
          ...
          .json({ error: StatusMessage.PROFILE_NOT_FOUND });
      }
    } catch (error) {
  ...
  editProfile = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const data: EditProfileInputDto = { ...req.body };
      ...
      if (!userId) {
        res
          ...
          .json({ error: StatusMessage.UNAUTHORIZED });
        return;
      }
  ...
  changePassword = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const { newPassword, oldPassword }: ChangePasswordInputDto = req.body;
  ...
  getPayment = async (req: AuthRequest, res: Response) => {
    try {
      ...
      const payment = await this._profileService.getPayment(
        userId as string,
        page,
      );
  ...
  notifications = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      const page = Number(req.params.page);
      const noti = await this._profileService.getNotifications(
        userId as string,
        page,
      );
  ...
  notificationsMarkRead = async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      const notifications = await this._profileService.notificationsMarkRead(
        userId as string,
      );
