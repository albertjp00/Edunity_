...
export class ProfileService implements IUserProfileService {
  ...
  async passwordChange(id: string, newPassword: string, oldPassword: string): Promise<boolean> {
    try {
      ...
      await this._userRepository.changePassword(id, hashedPassword);
      return true;
    } catch (error) {
      console.error(error);
      return false;
  ...
  async getPayment(userId: string , page:number): Promise<IPaymentDetailsService | null> {
    try {
      const result = await this._userRepository.getPayment(userId , page)
  ...
  async getNotifications(userId: string , page :number): Promise<INotifications | null> {
    try {
      const noti = await this._userRepository.getNotifications(userId , page)
      return noti
    } catch (error) {
      console.log(error);
      return null
  ...
  async notificationsMarkRead(userId: string): Promise<INotification[] | null> {
    try {
      const noti = await this._userRepository.notificationsMarkRead(userId)
      return noti
    } catch (error) {
      console.log(error);
      return null
  ...
  async subscriptionCheckRequest(id: string): Promise<ISubscription | boolean | null> {
    try {
      const user = await this._userRepository.getSubscriptionActive(id)
      return user
    } catch (error) {
      console.log(error);
      return null
